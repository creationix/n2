...
test("Encodes strings", () => {
    ...
    expect(toHex(encode("Hello, World!"))).toEqual('48656c6c6f2c20576f726c64214d');
    expect(toHex(encode("This is a longer string that exceeds 27 characters.")))
        .toEqual('2e41546869732069732061206c6f6e67657220737472696e67207468617420657863656564732032372063686172616374657273325c42371c');
...
test("Encodes binary data", () => {
    ...
    expect(toHex(encode(new Uint8Array([1, 2, 3, 4, 5])))).toEqual('010203040565');
    expect(toHex(encode(new Uint8Array([...Array(30).keys()]))))
        .toEqual('000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e7c');
})
...
test("Encodes arrays", () => {
    ...
    expect(toHex(encode(new Array(30).fill(0).map((_, i) => i))))
        .toEqual('1d3c1c3c1b3c1a3c193c183c173c163c153c143c133c123c113c103c0f3c0e3c3a38363432302e2c2a28262422202e9c');
})

test("Encodes objects", () => {
    expect(toHex(encode({}))).toEqual('80a1');
    expect(toHex(encode({ a: 1, b: 2 }))).toEqual('24226241614184a7');
    expect(toHex(encode({ foo: "bar", baz: [1, 2, 3] }))).toEqual('6261724326242283666f6f4362617a4388b1');
    expect(toHex(encode({ nested: { a: { b: { c: 3 } } } }))).toEqual('26634182a4624182a8614182ac6e65737465644687b5');
    expect(toHex(encode({ a: 1, b: { c: 2, d: [3, 4] }, e: "five" })))
        .toEqual('6669766544282682246441634184a92265416241614186b7');
})

test("Encodes with references", () => {
    expect(toHex(encode(["repeat", "repeat", "repeat"])))
        .toEqual('72657065617446c0c189');
    expect(toHex(encode({ a: "same", b: "same", c: "same" })))
        .toEqual('73616d6544c0c163416241614186ae');
    const obj = { key: "value" };
    expect(toHex(encode(obj))).toEqual('76616c7565456b65794384ab');
    expect(toHex(encode([obj, obj, obj]))).toEqual('76616c7565456b65794384abc0c18e');
})
