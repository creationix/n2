...
export function parse(input: string, filename = "[memory]"): any {
  ...
  function parseArray(): any[] {
    ...
    if (input[index] !== ']') {
      throw new SyntaxError(`Expected ']' at position ${toLocation(index, input, filename)}`);
    }
    index++; // Skip ']'
    return arr;
  }
      ...
      if (char === quoteType) {
        ...
        if (escapeChar === 'n') str += '\n';
          ...
          if (!/^[0-9a-fA-F]{4}$/.test(hex)) {
            throw new SyntaxError(`Invalid Unicode escape sequence at position ${index}`);
          }
  ...
  function parseBinary(): Uint8Array {
    ...
    while (index < length) {
      ...
      if (char === '>') {
        ...
        throw new SyntaxError(`Invalid character in binary data at position ${index}`);
      }
      index++;
    }
    if (hexString.length % 2 !== 0) {
      throw new SyntaxError(`Hex string must have an even length at position ${index}`);
    }
